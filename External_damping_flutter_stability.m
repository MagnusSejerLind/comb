...
for i_case = 1:2
    for i = 1:length(u_vec)
        ...
        C = [c_in_1 - chi * u * (gamma_1 + l) * (a + b) + c_hat*(a+b)*(l+1/2), -chi * u * gamma_2 * l * (a + b), 0, 0;
            -chi * u * gamma_2 * l * (a + b), c_in_2 - chi * u * gamma_2 ^ 2 * l * (a + b) + 1/2 * c_hat * l^2 *(a+b), 0, 0;
            chi * u * gamma_1 * xi * (a + b), 0, d_in_1 + 1/2 * c_hat * (a^2 - b^2), 0;
            u * chi * xi * l * (a + b), u * chi * xi * l ^ 2 * gamma_2 * (a + b), 0, d_in_2 + 1/2 * c_hat * l * (a^2-b^2) ;];
